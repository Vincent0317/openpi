    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        "top_image": "top_image",               # ���ӽ�ͼ��
                        "wrist_image": "wrist_image",       # �����ӽ�ͼ��
                        "state": "state",                   # ������״̬
                        "actions": "actions",               # ��������
                        "task": "prompt",                   # ��������
                    }
        ...
        # data_transforms = data_transforms.push(
        #     # inputs=[_transforms.DeltaActions(delta_action_mask)],
        #     # outputs=[_transforms.DeltaActions(delta_action_mask)],
        # )
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LeRobotAirbotDataConfig(
            repo_id="full_task",
            # repo_id="single_task/l2_lift_broccoli", # repo_id: ���ݼ��ĵ�ַ
            # repo_id="multi_task",
            base_config=DataConfig(
                local_files_only=True,  # �Ƿ�ֻʹ�ñ������ݼ�
                prompt_from_task=True, # prompt_from_task: �Ƿ�������л�ȡ��ʾ��
            ),
        ),
        save_interval=5000,
        batch_size = 64,
        num_workers = 8,
        resume = False,
        checkpoint_base_dir = "/mnt/ssd0/data/pi0_agent/zwc/checkpoints/pi0_airbot", # ��ѵ���õ���checkpoints�ĸ�Ŀ¼
        # ��S3�洢����Ԥѵ����pi0����ģ��Ȩ��
        weight_loader=weight_loaders.CheckpointWeightLoader("s3://openpi-assets/checkpoints/pi0_base/params"),
        # ѵ���ܲ���
    ...
    TrainConfig(
        ...
        num_workers = 16,
        resume = False,
        checkpoint_base_dir = "/mnt/ssd0/data/pi0_agent/zwc/checkpoints/pi0_fast_airbot",
    ...
    TrainConfig(
        name="pi0_airbot_low_mem_finetune_bear",
        # Here is an example of loading a pi0 model for LoRA fine-tuning.
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotAirbotDataConfig(
            repo_id="bear_agent",
        ...
        num_workers = 16,
        resume = True,
        checkpoint_base_dir = "/mnt/ssd0/data/pi0_agent/zwc/checkpoints/pi0_airbot_low_mem",
        weight_loader=weight_loaders.CheckpointWeightLoader("s3://openpi-assets/checkpoints/pi0_base/params"),
